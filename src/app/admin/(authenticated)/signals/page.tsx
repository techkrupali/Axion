...
import { Loader2, Eye, CheckCircle, PhoneCall, MoreHorizontal, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AdminSignalsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-[#e5e5e5]">Signals</h1>
        <p className="text-[#737373] mt-2 font-medium">Manage incoming diagnostic signals.</p>
      </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#e5e5e5]">{signal.name}</span>
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest border ${getStatusColor(signal.status)}`}>
                        {signal.status}
                      </span>
                    </div>
                    <span className="text-[#737373] text-sm">{signal.email}</span>
                    <span className="text-[#525252] text-[10px] mt-1">{new Date(signal.createdAt).toLocaleString()}</span>
                  </div>
                </div>
              ...
              {expanded === signal._id && (
                <div className="border-t border-[#2a2a2a] p-6 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-2">Contact</h4>
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-2">Context</h4>
                      <div className="space-y-1 text-sm">
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-2">Practices</h4>
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-3">Signals</h4>
                    <ul className="space-y-2">
                      {signal.signals.map((s, idx) => (
                        <li key={idx} className="flex items-center gap-2 text-sm text-[#e5e5e5]">
                    ...
                    {signal.rankedSignals && signal.rankedSignals.length > 0 && (
                      <div className="mt-4">
                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-3">Ranked Signals</h4>
                        <ol className="list-decimal list-inside space-y-2">
                          {signal.rankedSignals.map((s, idx) => (
                            <li key={idx} className="text-sm text-[#e5e5e5]">{s}</li>
                          ))}
                        </ol>
                      </div>
                    )}
                  ...
                  {signal.whyNow && (
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-3">Why Now</h4>
                      <p className="text-sm text-[#e5e5e5] bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4">
                        {signal.whyNow}
                      </p>
                    </div>
                  )}
                  ...
                  {signal.catchAllDetail && (
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#737373] mb-3">Additional Details</h4>
                      <p className="text-sm text-[#e5e5e5] bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4">
                        {signal.catchAllDetail}
                      </p>
                    </div>
