...
export interface Profile { name?: string; role?: string; experienceBand?: '0-3'|'4-7'|'8-12'|'13-20'|'20+'; }
...
const allocation = (answers: Answers, qid: string, ids: string[]) =>
  ids.reduce((s, id) => s + (((answers[qid] || {}) as Record<string, number>)[id] || 0), 0);
...
export function scoreDiagnostic(indexType: IndexType, answers: Answers, profile: Profile = {}) {
  ...
  const rawEdgeScore =
    judgmentDensity * weights.judgmentDensity! +
    (100 - outputExposure) * weights.outputProtection! +
    consequenceSignal * weights.consequenceSignal! +
  ...
  return {
    indexType, profile,
    score: round(interpretedScore),
    rawScore: round(rawEdgeScore),
    range: `${Math.max(0, Math.round(interpretedScore - 5))}-${Math.min(100, Math.round(interpretedScore + 5))}`,
    band: band(interpretedScore),
    archetype: archetype(interpretedScore, judgmentDensity, growthSignal, outputExposure),
    direction: direction(growthSignal),
    components: Object.fromEntries(
      Object.entries({ workTypeCompression, workTypeAIProof, judgmentWorkAllocation, stoScore, impactScore, timeHorizonScore, judgmentOwnership, thinkingOwnership, consequenceVisibility, edgeMirror, scopeEvolution, momentum, judgmentDensity, outputExposure, consequenceSignal, growthSignal })
...
function archetype(s: number, jd: number, g: number, oe: number) {
  if (s >= 75 && jd >= 70 && g >= 70) return 'Boundary Builder';
  if (s >= 75) return 'Structural Architect';
  if (s >= 50 && jd >= 65 && g < 50) return 'Position Defender';
  if (s >= 50) return 'Transition Navigator';
  if (oe >= 65) return 'Compression Exposed';
  return 'Execution Anchor';
}
function rankGaps(obj: Record<string, number>) {
  return Object.entries(obj).sort((a, b) => a[1] - b[1]).map(([dimension, score]) => ({ dimension, score: round(score) }));
}
function topKey(ans: Record<string, number> | undefined) {
  return Object.entries(ans || {}).sort((a: [string, number], b: [string, number]) => b[1] - a[1])[0]?.[0];
}
function crossSignals(answers: Answers) {
  const wt = topKey(answers.q1_workType as Record<string, number>);
  const sto = topKey(answers.q2_sto as Record<string, number>);
  const judgmentMode = topKey(answers.q5_judgment as Record<string, number>);
  const thinkingMode = topKey(answers.q6_thinking as Record<string, number>);
  const stoWork =
    sto === 'operational' && ['research', 'executing'].includes(wt ?? '') ? 'Operational + AI-dominant: highest compression exposure.' :
    sto === 'strategic' && ['framing', 'deciding', 'insighting'].includes(wt ?? '') ? 'Strategic + AI-proof: strongest protection signal.' :
    sto === 'strategic' && ['research', 'executing'].includes(wt ?? '') ? 'Strategic title / compressible activity mismatch.' :
    sto === 'operational' && ['framing', 'deciding', 'insighting'].includes(wt ?? '') ? 'Under-leveraged judgment at operational layer.' :
    'Mixed work-layer signal.';
  const jt = ['own', 'lead'].includes(judgmentMode ?? '') && ['original', 'adaptive'].includes(thinkingMode ?? '')
    ? 'Strong judgment-thinking ownership.'
    : 'Judgment-thinking gap: increase original framing and owned calls.';
  return { dominantWorkType: wt, dominantSTO: sto, stoWork, judgmentThinking: jt };
}

export type DiagnosticResult = ReturnType<typeof scoreDiagnostic>;
